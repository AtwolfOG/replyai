...
export function GenerationSettings() {
  ...
  useEffect(() => {
    if (isSuccess) {
      ...
      dispatch({type: "SET_SETTINGS", payload: {tone: data.default_tone, length: data.default_length, audience: data.default_audience, auto_copy: data.auto_copy}})
      console.log(data)
    }
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-(--space-4)">
            ...
            <div className="flex items-center gap-(--space-2)">
              ...
              <Switch onCheckedChange={(checked) => updateSettingsMutation({...state, auto_copy: checked})} size="default"/>
          </div>
  ...
  <DropdownMenuContent>
    <DropdownMenuGroup>
      ...
      {options.map((option, index) => (
        <DropdownMenuItem  className={option === value ? "bg-foreground/70 text-primary-foreground" : ""} key={index} onClick={() => onChange(option)}>{option}</DropdownMenuItem>
      ))}
    </DropdownMenuGroup>
  </DropdownMenuContent>
</DropdownMenu>
